        ...
        if self.sep_style == SeparatorStyle.SINGLE:
            ...
            if self.tokenizer is None:
                if self.version == "llama_v3":
                    self.tokenizer = AutoTokenizer.from_pretrained('meta-llama/Meta-Llama-3-8B-Instruct', trust_remote_code=True)
                elif self.version == "llava_llada":
                    tokenizer_id = self.tokenizer_id or os.environ.get("LLADA_V_TOKENIZER")
                    if not tokenizer_id:
                        raise ValueError("Set Conversation.tokenizer_id or LLADA_V_TOKENIZER for llava_llada.")
                    self.tokenizer = AutoTokenizer.from_pretrained(tokenizer_id, trust_remote_code=True)
                else:
                    raise ValueError("The tokenizer is not available. Make sure you have the necessary permissions.")
...
conv_llava_llama_3 = Conversation(
    ...
    tokenizer=safe_load_tokenizer("/data_storage/lbw/shared/pretrained_models/Meta-Llama-3-8B-Instruct"),
